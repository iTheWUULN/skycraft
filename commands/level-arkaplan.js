...
module.exports = {
  ...
  run: async(client, interaction, db, Rank, AddRank, RemoveRank) => {
    ...
    if(!interaction.member.permissions.has(PermissionsBitField.ManageMessages)) {
      return interaction.followUp({ content: "<:carpi:1136592790843838505> | Mesajları Yönet Yetkin Yok!" })
    }

    db.set(`arkaplan_${interaction.guild.id}`, link)

    interaction.reply("<:tik:1136344350830366740> | Başarılı bir şekilde arka plan ayarlandı!")
